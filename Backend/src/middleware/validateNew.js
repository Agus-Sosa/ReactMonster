...
export const validateNew=(req, res, next)=> {
    ...
    if (!title || title.length < 3){
       return res.status(404).json({status: "error", message: "El titulo debe tener al menos 3 caracteres."})
    }
    if (!resume || resume.length < 20 || resume.length > 250){
       return res.status(404).json({status: "error", message: "El resumen debe ser mayor a 20 caracteres y no mayor a 250."})
    }
    if (!content || content.length < 20 ){
       return res.status(404).json({status: "error", message: "El contenido de la noticia debe ser mayor a 20 caracteres y."})
    }
    if (haveBadWord(title)){
       return res.status(404).json({status: "error", message: "El titulo contiene palabras no permitidas."})
    }
    if (haveBadWord(resume)){
       return res.status(404).json({status: "error", message: "El resumen contiene palabras no permitidas."})
    }
    if (haveBadWord(content)){
       return res.status(404).json({status: "error", message: "El contiene contiene palabras no permitidas."})
    }
